...
interface Vendor { id: string; shopName: string; status: string; }
    ...
    (async () => {
      try {
        ...
        if (v.status === 'APPROVED') {
          const p = await api<Product[]>('/api/products/vendor/mine');
          setProducts(p);
        }
      } catch (e: any) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <KpiCard label="Today's revenue" value="₹0" hint="Updates as orders are paid" />
        <KpiCard label="Orders to ship" value="0" hint="Mark shipped from Orders" accent="brand" />
        <KpiCard label="Active listings" value={activeCount} hint={`${products.length} total`} accent="success" />
        <KpiCard label="Stock alerts" value={lowStock + outOfStock} hint={`${outOfStock} out · ${lowStock} low`} accent="warn" />
      </div>
