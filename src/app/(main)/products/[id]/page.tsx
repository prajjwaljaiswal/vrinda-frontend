...
export default function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    if (token) {
      api<{ canReview: boolean; alreadyReviewed: boolean }>(
        `/api/reviews/can-review/${params.id}`
      )
        .then((d) => {
  ...
  // Min/max across combos (for "from ₹X" / "₹X – ₹Y" range display).
  ...
  // For single-variation products, surface each option's price next to its chip
  // (Etsy-style). For multi-variation we'd need a 2-D price table, so we skip it.
    ...
    <div className="max-w-container mx-auto px-6 py-8">
        ...
        <div className="lg:sticky lg:top-32 self-start space-y-5">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {product.featured && (
                <span className="text-[10px] uppercase tracking-wide font-semibold px-2 py-0.5 rounded-full bg-brand-50 text-brand-700">Featured</span>
              )}
      ...
      <div className="mt-16 border-t border-line pt-10">
        <div className="flex items-center justify-between mb-8">
          ...
          {alreadyReviewed && (
            <span className="text-sm text-success font-medium">✓ You've reviewed this product</span>
          )}
        </div>
            ...
            <div className="space-y-2 justify-center flex flex-col">
              {[5, 4, 3, 2, 1].map((star) => (
                <RatingBar
                  key={star}
                  star={star}
                  count={reviewsData.ratingBreakdown[star] ?? 0}
                  total={reviewCount}
                />
              ))}
            </div>
          ...
          <div className="mb-10 p-6 bg-white border border-line rounded-lg">
            ...
            <form onSubmit={submitReview} className="space-y-4">
              ...
              <div>
                ...
                <label className="block border-2 border-dashed border-line rounded-md p-4 text-center cursor-pointer hover:border-brand-600 transition">
                  ...
                  <p className="text-sm text-ink-700">
                    <span className="text-brand-700 font-semibold">Click to upload</span> images or videos
                  </p>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="button" onClick={() => { setShowForm(false); setReviewErr(''); }} className="btn-secondary !px-6">
                  Cancel
                </button>
              </div>
            </form>
